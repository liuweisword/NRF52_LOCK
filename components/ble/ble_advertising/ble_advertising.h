 ...
 *
 * @param[in] p_advdata     Advertising data: name, appearance, discovery flags, and more.广播数据
 * @param[in] p_srdata      Scan response data: Supplement to advertising data.扫描反馈数据
 * @param[in] p_config      Select which advertising modes and intervals will be utilized.选择广播模式与广播间隔
 * @param[in] evt_handler   Event handler that will be called upon advertising events.广播事件后被调用的事件操作
 * @param[in] error_handler Error handler that will propogate internal errors to the main applications.内部出现错误时执行错误处理操作
 *
